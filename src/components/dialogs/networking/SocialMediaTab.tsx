...
import { SocialMediaLink, SocialMediaState } from './types';
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start space-x-3">
              <div className="flex h-5 items-center space-x-2">
                <Checkbox 
                  id="twitter" 
                  checked={socialMedia.twitter?.enabled || false}
                  onCheckedChange={(checked) => 
                    handleSocialMediaChange('twitter', 'enabled', checked === true)
                  }
                />
              ...
              {socialMedia.twitter?.enabled && (
                <Input
                  value={socialMedia.twitter.url}
                  onChange={(e) => handleSocialMediaChange('twitter', 'url', e.target.value)}
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start space-x-3">
              <div className="flex h-5 items-center space-x-2">
                <Checkbox 
                  id="youtubeMusic" 
                  checked={streamingPlatforms.youtubeMusic?.enabled || false}
                  onCheckedChange={(checked) => 
                    handleStreamingChange('youtubeMusic', 'enabled', checked === true)
                  }
                />
                <Label htmlFor="youtubeMusic" className="font-medium cursor-pointer">
                  YouTube Music
                </Label>
              </div>
              {streamingPlatforms.youtubeMusic?.enabled && (
                <Input
                  value={streamingPlatforms.youtubeMusic.url}
                  onChange={(e) => handleStreamingChange('youtubeMusic', 'url', e.target.value)}
              ...
              {socialLinks
                .filter(link => 
                  !['instagram', 'facebook', 'youtube', 'tiktok', 'twitter', 
                    'spotify', 'deezer', 'applemusic', 'soundcloud', 'amazonmusic', 'youtubemusic']
                ...
                .map((link, index) => (
                  <Badge key={index} variant="outline" className="pl-2 flex items-center gap-1 bg-background/80">
                    ...
                    <span className="max-w-[150px] truncate">{link.platform}: {link.url}</span>
                    <Button
                      type="button"
                      variant="ghost" 
                      size="sm"
            ...
            <div className="grid gap-2 flex-1">
              ...
              <datalist id="platform-suggestions">
                {[...socialNetworksDB, ...platformsDB]
                  .filter(platform => 
                    !['Instagram', 'Facebook', 'YouTube', 'TikTok', 'Twitter',
                      'Spotify', 'Deezer', 'Apple Music', 'SoundCloud', 'Amazon Music', 'YouTube Music']
                      .includes(platform)
                  )
                  .map((platform, idx) => (
